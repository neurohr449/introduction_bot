    ...
    try:
        ...
        if not user_row:
            update_data = {
                ...
                'Статус': status or "Начал модуль",
                'Название блока': block_name or "",
                'Название модуля': module_name or "",
                'День': current_day,
                'Месяц': current_month,
                'Год': current_year
            }
        ...
        if interview_date is not None:
            update_data['Дата встречи'] = interview_date
        if interview_time is not None:
            update_data['Время встречи'] = interview_time
        if qa_data is not None:
            update_data['Вопросы и ответы'] = qa_data
        if user_score is not None:
            update_data['Баллы'] = user_score
        if block_name is not None:
            update_data['Название блока'] = block_name
        if module_name is not None:
            update_data['Название модуля'] = module_name
        ...
        if user_row:
            ...
            row_values = [
                ...
                current_values.get('Название блока', ''),      # F
                current_values.get('Название модуля', ''),      # G
                current_values.get('ФИО', ''),                    # H
                current_values.get('Номер', ''),                  # I
                current_values.get('Дата встречи', ''),     # J
                current_values.get('Время встречи', ''),    # K
                current_values.get('Баллы', ''),                  # L
                current_values.get('AI комент', ''),              # M
                current_values.get('Вопросы и ответы', ''),       # N
                current_values.get('День', ''),                   # O
                current_values.get('Месяц', ''),                  # P
                current_values.get('Год', '')                     # Q
            ]
            
            await asyncio.to_thread(sheet.update, f'A{user_row}:Q{user_row}', [row_values])
        else:
            ...
            new_row = [
                ...
                status or "Начал модуль",      # E Статус
                block_name or "",              # F Название блока
                module_name or "",             # G Название модуля
                full_name or "",               # H ФИО
                phone_number or "",            # I Номер
                interview_date or "",          # J Дата встречи
                interview_time or "",          # K Время встречи
                user_score or "",              # L Баллы
                gpt_response or "",            # M AI комент
                qa_data or "",                 # N Вопросы и ответы
                current_day,                   # O День
                current_month,                 # P Месяц
                current_year                   # Q Год
            ]
...
async def get_table_data(sheet_id, sheet_range, state: FSMContext,):
    range_name = f"A{sheet_range}:FB{sheet_range}"
    ...
    await state.update_data(
        ...
        video_8 = row_data[62],
        video_9 = row_data[63],
        video_10 = row_data[64],
        video_11 = row_data[65],
        video_12 = row_data[66],
        video_13 = row_data[67],
        video_14 = row_data[68],
        video_15 = row_data[69],
        video_16 = row_data[70],
        video_17 = row_data[71],
        video_18 = row_data[72],
        video_19 = row_data[73],
        video_20 = row_data[74],
        video_21 = row_data[75],
        video_22 = row_data[76],
        video_23 = row_data[77],
        video_24 = row_data[78],
        video_25 = row_data[79],
        video_26 = row_data[80],
        video_27 = row_data[81],
        video_28 = row_data[82],
        video_29 = row_data[83],
        video_30 = row_data[84],
        video_31 = row_data[85],
        video_32 = row_data[86],
        video_33 = row_data[87],
        video_34 = row_data[88],
        video_35 = row_data[89],
        video_36 = row_data[90],
        video_37 = row_data[91],
        video_38 = row_data[92],
        video_39 = row_data[93],
        video_40 = row_data[94],
        questionare = row_data[95],
        ...
        q39 = row_data[134],
        q40 = row_data[135],
        meeting = row_data[136],


        closing_text=row_data[137],
        promt=row_data[138],
        promt_2=row_data[139],
        target_score = row_data[140],
        test_tiser=row_data[141],
        processing_answers=row_data[142],
        result_yes=row_data[143],
        result_no=row_data[144],
        confurmation_message=row_data[145],
        notification_pd=row_data[146],
        notification_hour=row_data[147],
        notification_now=row_data[148]
        )
